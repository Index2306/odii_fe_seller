...
export default memo(function SaveCustomer({
  customerInfo,
  setCustomerInfo,
  saveType,
  onCancel,
  ...
  const isCreateNew = saveType === CREATE_CUSTOMER_TYPE;
  ...
  const fetchListWard = district => {
    getLocation({ parent_id: district, type: 'ward' }).then(res => {
  ...
  const saveCustomer = async () => {
    setLoading(true);
    const requestUrl =
      'user-service/admin/customer' +
      (isCreateNew ? '' : `/${customerInfo.id}`);
    ...
    try {
      ...
      if (response.is_success && isCreateNew) {
      ...
      title={
        <>
          <div className="modal-title__main">
            {isCreateNew ? 'Tạo khách hàng mới' : 'Cập nhật khách hàng'}
          </div>
          <div className="modal-title__desc">
            Vui lòng nhập các thông tin dưới đây để lưu khách hàng
          </div>
        </>
      }
      footer={null}
      ...
      <Spin spinning={isLoading}>
        <Form name="save-customer" {...formItemLayout} form={form}>
          <div className="form-group">
            <Row>
              <Col span={24}>
                <Form.Item name="customer_full_name" label="Họ & tên">
              ...
              <Col xs={24} sm={24} md={24} lg={12} xl={12}>
                <Form.Item
                  ...
                  rules={[
                    {
                      type: 'email',
                      message: t('user.validate.emailValidate'),
                    },
                    {
                      required: true,
                      message: t('user.validate.emailRequired'),
                    },
                  ]}
                >
                  <Input placeholder="Nhập email" onChange={updateValidForm} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={24} md={24} lg={12} xl={12}>
                <Form.Item
                  ...
                  label="Số điện thoại"
                  rules={[
                    { required: true, message: t('user.validate.phone1') },
                    {
                      validator: (_, value) => {
                        if (!value || /^\d+$/.test(value))
                          return Promise.resolve();
                        return Promise.reject(
                          new Error(t('user.validate.phone2')),
                        );
                      },
                    },
                  ]}
                >
                  <Input placeholder="Nhập SĐT" onChange={updateValidForm} />
                </Form.Item>
              </Col>
            </Row>
          </div>
...
export const SaveCustomerModal = styled(Modal)`
  min-height: 550px;
  top: 50%;
